 ...
 * POST /api/auth/upgrade      — upgrade plan (MUST be called from payment webhook only)
  ...
  return (req, res, next) => {
    ...
    for (const [field, rules] of Object.entries(schema)) {
      ...
      if (val !== undefined && val !== null && val !== "") {
        ...
        // SECURITY FIX: enforce password complexity in backend
        if (rules.complexity && !/(?=.*[A-Z])(?=.*[0-9])/.test(val))
          errors.push(`${field} must contain at least one uppercase letter and one number.`);
      }
    }
...
router.post("/register",
  validateBody({
    ...
    // SECURITY FIX: enforce 8-char minimum and complexity in ONE place (here)
    password:    { required:true,  type:"string", minLength:8,   maxLength:128, complexity:true },
...
router.put("/me", requireAuth,
  ...
  try {
    const { rows } = await pool.query(
      ...
      [name || null, institution !== undefined ? (institution || "") : null, language || null, req.user.id]
    );
...
// SECURITY FIX: This endpoint is now restricted to internal/webhook calls only.
// In production, your payment gateway (Stripe/Chapa) webhook should POST here
// with the UPGRADE_WEBHOOK_SECRET header, NOT users calling it directly.
//
// For development/demo only: set ALLOW_DIRECT_UPGRADE=true in .env to allow
// direct calls (e.g. for testing without a payment provider).
router.post("/upgrade", requireAuth, async (req, res) => {
  const isDemoMode = process.env.ALLOW_DIRECT_UPGRADE === "true";
  const webhookSecret = req.headers["x-upgrade-webhook-secret"];
  const expectedSecret = process.env.UPGRADE_WEBHOOK_SECRET;

  if (!isDemoMode) {
    if (!expectedSecret || webhookSecret !== expectedSecret) {
      return res.status(403).json({
        error: "Plan upgrades must be initiated through the payment gateway webhook.",
      });
    }
  }
  ...
  try {
    ...
    console.log(`[auth] Upgraded user id=${user.id} → ${plan} (ref: ${payment_reference || "none"})`);
    res.json({
      user:        safeUser(user),
      accessToken,
