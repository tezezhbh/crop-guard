/**
 * server.js — CropGuard AI Backend v3 (Security-Fixed)
 *
 * SECURITY FIXES APPLIED:
 *   - CSP enabled (was false)
 *   - CORS no longer bypasses all origins in development mode
...
// ── Security ──────────────────────────────────────────────
// SECURITY FIX: CSP is now enabled with appropriate directives.
app.use(helmet({
...
// ── CORS ──────────────────────────────────────────────────
// SECURITY FIX: Always enforce origin allowlist. Development localhost entries
// are included in the allowlist directly — no blanket bypass for NODE_ENV=development.
...
app.use(cors({
  ...
  allowedHeaders: ["Content-Type", "Authorization", "x-upgrade-webhook-secret"],
  credentials:    true,
}));
...
async function start() {
  ...
  if (ENV === "production") {
    if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 32) {
      console.error("[startup] FATAL: JWT_SECRET must be set and >= 32 chars in production");
      process.exit(1);
    }
    if (!process.env.DATABASE_URL || process.env.DATABASE_URL.includes("changeme")) {
      console.error("[startup] FATAL: DATABASE_URL must be configured in production");
      process.exit(1);
    }
    if (!process.env.GROQ_API_KEY) {
      console.warn("[startup] GROQ_API_KEY not set — AI chat will be unavailable");
    }
    if (process.env.ALLOW_DIRECT_UPGRADE === "true") {
      console.warn("[startup] WARNING: ALLOW_DIRECT_UPGRADE=true — disable this in production");
    }
  }
